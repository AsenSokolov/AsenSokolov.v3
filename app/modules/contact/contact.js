define(function(require) {
    ...
    var contactService = require('app/modules/contact/contact-service.js');
    contact.factory('contactService', contactService);

    contact.controller('ContactController',['$scope','coverParams','$window','$animate','$http', 'metaTags', 'pageLoading', 'contactService',
        function ($scope, coverParams, $window, $animate, $http, metaTags, pageLoading, contactService) {
            pageLoading.setLoadingStatus(true);

            contactService.getInfo().then(function(inf) {
                $scope.contactData = inf;

                pageLoading.setLoadingStatus(false);

                coverParams.setCoverTitle($scope.contactData.coverTitle);
                coverParams.setCoverSubTitle($scope.contactData.coverDescription);
                coverParams.setCoverImage($scope.contactData.coverImage);

                metaTags.setPageTitle($scope.contactData.title);
                metaTags.setPageDescription($scope.contactData.descr);
                metaTags.setPageKeywords($scope.contactData.keywords);
                metaTags.setFbImage($scope.contactData.fbImage);
                metaTags.setTwImage($scope.contactData.twImage);
            });
