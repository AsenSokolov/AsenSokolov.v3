...
class Image {
	...
	// 
	function cropImage($path, $dw=100, $dh=100, $thumb=false)
	{
		...
		$config['master_dim']		= ($ow > $oh ? 'height' : 'width');
		$config['width'] 			= $width;
		...
		list($nw, $nh) = getimagesize(($thumb ? $thumb : $path));
			
		$config['quality'] 			= '100%';
		$config['source_image'] 	= ($thumb ? $thumb : $path);
		$config['width']  			= $dw;
		$config['height'] 			= $dh;
		$config['maintain_ratio'] 	= false;
		$config['master_dim']		= ($nw > $nh ? 'height' : 'width');
		$config['x_axis'] 			= round(($nw-$dw)/2);
		$config['y_axis'] 			= round(($nh-$dh)/2);
		
		$this->initialize($config); 
		$this->crop();
		$this->clear();
		
		chmod($config['source_image'], 0644);
		
		return ($thumb ? $thumb : $path);
	}
	
	function cropImage_1($path, $dw=100, $dh=0, $thumb=false)
	{
		list($ow, $oh) = getimagesize($path);
